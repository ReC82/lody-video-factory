...
# -- import / export partiel des lieux (#57) --------------------------------------------------------------------------
def _transfer_prefix(project_id: str) -> str:
    return f"locimp_{project_id}"


def _export_bytes(repo: LocationRepository, project_id: str) -> bytes:
    payload = resource_transfer.export_locations(repo.list_for_project(project_id))
    return json.dumps(payload, ensure_ascii=False, indent=2).encode("utf-8")


def _template_bytes() -> bytes:
    return json.dumps(resource_transfer.location_template(), ensure_ascii=False, indent=2).encode("utf-8")


def _confirm_location_import(repo: LocationRepository, project_id: str,
                             preview: resource_transfer.ResourcePreview) -> None:
    t = _transfer_prefix(project_id)
    try:
        created = resource_transfer.commit_location_import(repo.db_path, project_id, preview)
    ...
    names = ", ".join(location.name for location in created)
    nav.flash("success", f"{len(created)} lieu(x) importé(s) : {names}." if created
             else "Fichier importé : aucun lieu à ajouter.")
...
def _render_transfer_preview(preview: resource_transfer.ResourcePreview) -> None:
    ...
    st.markdown(
        ...
        f'{len(preview.item_names)} lieu(x) à importer : {esc(names)}</div>', unsafe_allow_html=True,
    )
    if preview.warnings:
...
def _render_import_export(repo: LocationRepository, project: Project) -> None:
    t = _transfer_prefix(project.id)
    with st.container(key="loc_import_export_card"):
        st.markdown('<p class="card-eyebrow">Import / Export</p>', unsafe_allow_html=True)
        st.caption("Exporte les lieux de ce projet, ou importe un fichier pour en ajouter — sans jamais toucher "
                   "aux autres réglages du projet ni écraser un lieu déjà existant.")
        with st.container(horizontal=True, key=f"{t}_export_actions"):
            st.download_button("Exporter les lieux", data=_export_bytes(repo, project.id),
                               file_name=f"lody-lieux-{slugify(project.name, 40)}.json",
                               mime="application/json", key=f"{t}_export")
            st.download_button("Télécharger un modèle", data=_template_bytes(),
                               file_name="lody-modele-lieux.json", mime="application/json", key=f"{t}_template")

        uploaded = st.file_uploader("Importer des lieux (fichier .json)", type=["json"], key=f"{t}_uploader")
        if uploaded is not None:
            existing = repo.list_for_project(project.id)
            preview = resource_transfer.preview_location_import(
                uploaded.getvalue(), existing_names=[location.name for location in existing],
                existing_primary_count=sum(1 for location in existing if location.is_primary))
            _render_transfer_preview(preview)
            st.button("Confirmer l’import", type="primary", icon=":material/check:", key=f"{t}_confirm",
                      disabled=not preview.is_valid, on_click=_confirm_location_import,
