...
def _prompt(excerpt: str, style: str, vertical: bool) -> str:
    text = excerpt if len(excerpt) <= MAX_EXCERPT else excerpt[: MAX_EXCERPT - 1].rsplit(" ", 1)[0] + "…"
    parts = []
    if style.strip():
        parts.append(f"Style visuel : {style.strip().rstrip('.')}.")
    parts.append(f"Illustration cinématographique d’une scène qui accompagne ce passage : « {text} ».")
    parts.append("Composition verticale, un sujet clair, lumière soignée." if vertical else "Composition claire, un sujet net, lumière soignée.")
    parts.append("Aucun texte, aucun sous-titre, aucun logo, aucune marque, aucun filigrane.")
    return " ".join(parts)[:MAX_PROMPT]


def build_storyboard(script: str, scene_count: int, *, visual_style: str = "", aspect: str = "9:16",
                     narration_pace: str = "normal") -> list[Scene]:
    ...
    for index, group in enumerate(_group(sentences, min(scene_count, MAX_SCENES)), start=1):
        ...
        scenes.append(Scene(index, narration, _prompt(narration, visual_style, aspect == "9:16"), seconds))
    return scenes
